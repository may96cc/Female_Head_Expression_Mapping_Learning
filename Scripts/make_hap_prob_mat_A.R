...
# Get all datasets in the A package
all_datasets <- data(package = "DSPRqtlDataA")$results[, "Item"]
...
for(i in seq_along(all_datasets)) {
  ...
  # Load dataset
  data(list = ds_name, package = "DSPRqtlDataA")
  df <- get(ds_name)
  
  # Keep only ril + AA1–AA8
  df_sub <- df[, c("ril", paste0("AA", 1:8))]
  
  # Rename columns: chrom_position_AAi
  base_name <- sub("^A_", "", ds_name)
  setnames(df_sub, old = paste0("AA", 1:8), new = paste0(base_name, "_AA", 1:8))
...
fwrite(merged_dt, "A_RILS_Hap_Probs.csv")
